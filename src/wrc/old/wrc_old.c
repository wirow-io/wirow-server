...
static iwrc _worker_send(struct _msg *m) {
  iwrc rc = 0;
  struct wrc_msg *mm = &m->mm;
  struct _worker *w = _worker_find_ref(mm->worker_id);

  if (!w) {
    rc = GR_ERROR_RESOURCE_NOT_FOUND;
    iwlog_ecode_warn(rc, "WRC worker:%d not found or worker in shutdown pending state", mm->worker_id);
    goto finish;
  }
